...
class GPUController:
    ...
    def execute_gpu_service(self, input_dir: str, wav_file: str, json_file: str) -> Optional[str]:
        """Execute GPU service with direct file transfer"""
        try:
    ...
    def retrieve_results(self, work_dir: str, local_dir: str, original_files: Tuple[str, str], max_retries: int = 3, initial_delay: int = 5) -> bool:
        """Retrieve processed files with retries"""
        try:
            print("\n📥 Retrieving results...")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    rsync_cmd = [
                        ...
                        '--progress',
                        '-e', f'ssh -p {self.config["port"]}',
                        f"{self.config['host']}:{work_dir}/transcript_*",
                        transcripts_dir
                    ]
                    ...
                    if not found_files:
                        raise Exception("No transcript files found in the download")
                    
                    logging.info(f"Successfully downloaded files: {', '.join(found_files)}")

                    # Cleanup remote directory if needed
